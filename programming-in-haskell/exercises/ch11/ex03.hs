...
bestmove g p = head grids
  where
    tree = prune depth (gametree g p)
    Node (_, best) ts = minimax tree
    moves = filter (\(Node (_, p') _) -> p' == best) ts
    movesDepth = zip moves (map treedepth moves)
    sorted = sortBy (\(_, dl) (_, dr) -> compare dl dr) movesDepth
    grids = map (\(Node (g, _) _, _) -> g) sorted

treedepth :: Tree (Grid, Player) -> Int
treedepth (Node _ []) = 0
